...
#include <windows.h>
#include <ctime>
#include "blinktext.h" //written by user
...
void menu :: main_menu(void)
{
    ...
    {
        system("cls") ;
        TextBlink("S A L E S  M A N A G E M E N T  S Y S T E M", 50, 1, RGB(0, 255, 0), RGB(0, 0, 0));
        gotoxy(10,3) ;
        setColor(11);
        cout<<"*************************************************************" ;
        gotoxy(10,23) ;
...
void product :: list_of_item(void)
{
    ...
    gotoxy(3,4) ;
    setColor(11);
    cout <<"ITEMCODE   ITEM NAME           ITEM COST      ITEM PRICE" ;
    gotoxy(2,5) ;
    cout <<"***********************************************************" ;
    setColor(15);
    while (file.read((char *) this, sizeof(product)))
...
{   system("cls") ;
    char t_code[5], ch ;
    int t, tcode ;
    gotoxy(71,2) ;cout <<"<0>=Exit" ;
    gotoxy(3,25) ;
    cout <<"Press <ENTER> to see the list" ;
    gotoxy(5,3) ;setColor(11);
...
void product :: purchase(void)
{
    ...
    do
    {
        ...
        gotoxy(5,3) ;
        setColor(11);
        cout <<"Enter Item Code of the item to Purchase : " ;
        gets(t_code) ;
        t = atoi(t_code) ;
        tcode = t ;
        if (t_code[0] == '0')
        ...
        do
        {
            ...
            gotoxy(3,25) ;
            setColor(15);
            cout <<"ENTER QUANTITY TO BE PURCHASED IN Kg./count" ;
            gotoxy(5,8) ;
            cout <<"Quantity : " ;
            cin>>t_quantity ;
            qty=t_quantity ;
...
{   system("cls") ;
    ...
    cout <<"------------------------------------------------------------";
    gotoxy(8,9) ;
    cout <<"Itemcode  Item name           Cost    Price    Qty    Total";
    gotoxy(8,10) ;
    cout <<"------------------------------------------------------------";
    int row=11 ;
    fstream file ;
    file.open("BILL.DAT", ios::in) ;
    file.seekg(0) ;
    while (file.read((char *) this, sizeof(account)))  //while (file.read((char *) this, sizeof(account)) != 0 )
    {
        if (billno == t_billno)
        {
            gotoxy(8,5) ;
